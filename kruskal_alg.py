...
# первый этап работы алгоритма (соединение вершин, если хотя бы одна вершина изолирвана)
for r in Rs:
    if r[1] not in U or r[2] not in U: # проверка для исключения циклов в остове
        if r[1] not in U and r[2] not in U: # если обе вершины не соединены, то
            ...
            if not D.get(r[1]): # если в словаре нет первой вершины
                ...
                D[r[1]].append(r[2]) # иначе, всё то же самое делаем со второй вершиной
                D[r[2]] = D[r[1]] 
...
# второй этап: соединяем разные группы соединённых вершин
